...
function puzzleBlock(unit,s,esc){
  ...
  return `<div class="puzzle-card"><span class="mini-label">문장 배열 · SENTENCE BUILDER</span><div class="puzzle-heading"><h3>낱말을 눌러 대화의 문장을 완성해 보세요.</h3><button class="secondary" data-action="journey-tts" data-text="${esc(ordered.join(' '))}" aria-label="배열할 문장 듣기">🔊 문장 듣기</button></div><div class="puzzle-answer">${selected.length?selected.map(i=>`<span>${esc(mixed[i])}</span>`).join(''):'여기에 문장이 만들어져요.'}</div><div class="puzzle-chips">${mixed.map((token,i)=>`<button data-action="journey-token" data-index="${i}" ${selected.includes(i)?'disabled':''}>${esc(token)}</button>`).join('')}</div>${s.puzzleChecked?`<div class="activity-feedback ${selected.map(i=>mixed[i]).join(' ')===ordered.join(' ')?'success':''}">${selected.map(i=>mixed[i]).join(' ')===ordered.join(' ')?'✓ 문장을 잘 만들었어요!':'순서를 다시 살펴보세요.'}</div>`:''}<div class="puzzle-actions"><button class="secondary" data-action="journey-puzzle-clear">다시 배열하기</button><button class="small-primary" data-action="journey-puzzle-check" ${selected.length!==ordered.length?'disabled':''}>문장 확인</button></div></div>`;
}
